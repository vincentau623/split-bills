import { Stack, Typography, Button, Modal, Box, TextField, FormControl, InputLabel, MenuItem, Select, SelectChangeEvent, FormControlLabel, Checkbox } from "@mui/material";
...
const FinalPaymentSection = (props: { taxRate: number }) => {
    ...
    const [tempFinalPayment, setTempFinalPayment] = useState<FinalPayment>(bill.finalPayment);
    ...
    const hanldeTipsToSplitChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        setTempFinalPayment({
            ...tempFinalPayment,
            tipsToSplit: event.target.checked,
            tipsPaidByName: event.target.checked ? '' : tempFinalPayment.tipsPaidByName
        });
    }

    const handleTipsPaidByChange = (event: SelectChangeEvent) => {
        setTempFinalPayment({ ...tempFinalPayment, tipsPaidByName: event.target.value });
    }

    const handleFinalPaidChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        // if final price change update tips
        setTempFinalPayment({
            ...tempFinalPayment,
            tips: parseFloat(event.target.value)
                ? parseFloat(event.target.value) - parseFloat(tempFinalPayment.totalPrice.toFixed(2))
                : 0,
            finalPaid: parseFloat(event.target.value),
        });
    }
    ...
    const handlePaidByChange = (event: SelectChangeEvent) => {
        setTempFinalPayment({ ...tempFinalPayment, paidByName: event.target.value });
    }
    ...
    return (
        <>
            <Stack spacing={2}>
                ...
                <div>Tips: ${bill.finalPayment.tips.toFixed(2)} ({bill.finalPayment.tipsToSplit ? `Will be splitted` : `Pay by ${bill.finalPayment.tipsPaidByName}`})</div>
                <div>Final Paid: ${bill.finalPayment.finalPaid.toFixed(2)}</div>
                <div>Paid by: {bill.finalPayment.paidByName}</div>
                <Button variant="contained" onClick={handleFinalPaymentModalOpen}>Edit Final Payment</Button>
            </Stack>
            {/* Final Payment Modal */}
            ...
            >
                <Box sx={modalBoxStyle}>
                    <Stack spacing={2}>
                        ...
                        <TextField id="finalPayment-tips"
                            label="Tips"
                            variant="filled"
                            type="number"
                        ...
                        >
                            <FormControlLabel control={
                                <Checkbox checked={tempFinalPayment.tipsToSplit} onChange={hanldeTipsToSplitChange} />
                            } label="To split" />
                            {!tempFinalPayment.tipsToSplit &&
                                <FormControl sx={{ minWidth: '150px' }} variant="filled" >
                        ...
                        <TextField id="finalPayment-finalPrice"
                            label="Final Paid"
                            variant="filled"
                            type="number"
